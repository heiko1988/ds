...
(function() {
    ...
    // === MAP-MOVER INTEGRATION (FIXED: Kontextmenü für linke Klicks – stopPropagation entfernt) ===
    function integrateMapMover() {
        const waitForMapMover = setInterval(() => {
            ...
            if (mapMover) {
                ...
                // Nur für Rechtsklick (Kontextmenü): Browser-Menü blocken
                ...
                // Linker Klick: Voll durchlassen (kein stopPropagation, um TW-Menü zu erlauben)
                mapMover.addEventListener('click', (e) => {
                    if (e.button === 0) { // Linker Klick
                        debugLog('Linker Klick auf Map-Mover durchgelassen (kein stop).');
                    }
                }, true);
