...
*dy);
        ...
        Object.entries(allAttacksByProfile).forEach(([profileName, data], index) => {
            ...
            data.attacks.forEach(attack => {
                html += `<tr><td>${attack.village}</td><td>${attack.playerName}</td><td>${attack.coords}</td><td>${attack.speed}</td><td>${attack.dist}</td><td>${attack.runtimeStr}</td><td>${attack.startTimeStr}</td></tr>`;
            });
